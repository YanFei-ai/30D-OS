...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
	...
	for (h = h0; h <= ctl->top; h++) {
		sht = ctl->sheets[h];
        sid = sht - ctl->sheet0;   //将去图层的基址作为图层号使用
		buf = sht->buf;
...
//平移，不会改变图层的高度,当发生上下层的变化时，我们需要重新写map
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
	...
	if (sht->height >= 0) { //正在显示的图层
		...
		sheet_refreshsub(ctl, vx0, vy0, vx0 + sht->bxsize, vy0 + sht->bysize, sht->height, sht->height); //按新图层的西南西刷新画面
	}
	return;
}
