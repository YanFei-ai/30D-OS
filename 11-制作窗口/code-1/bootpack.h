...
struct SHEET {
	...
	int vx0, vy0;			//vx0*vy0 图层在画面上的位置坐标
	int col_inv;			//色号
	int height;				//height表示该图层在第几层
	int flags;				//存放图层的设定值，是否使用等
	struct SHTCTL *ctl;		//指定，该图层属于哪个ctl
};
...
void sheet_refresh(struct SHTCTL *ctl, struct SHEET *sht,  int bx0, int by0, int bx1, int by1);    //刷新图层的排列
...
void sheet_refresh(struct SHEET *sht,  int bx0, int by0, int bx1, int by1);    //刷新图层的排列
void sheet_slide(struct SHEET *sht, int vx0, int vy0);	//平移，不会改变图层的高度
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1);
