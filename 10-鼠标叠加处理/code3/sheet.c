...
void sheet_refresh(struct SHTCTL *ctl, struct SHEET *sht,  int bx0, int by0, int bx1, int by1){     //刷新全部图层的排列
...
void sheet_refreshsub(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1){     //刷新一个范围内图层的排列 vx0-vy1
    struct SHEET *sht;
    ...
    for(h = 0; h <= ctl->top; h++){
        ...
        for (by = 0; by < sht->bysize; by++) {
            ...
            for (bx = 0; bx < sht->bxsize; bx++) {
                vx = sht->vx0 + bx;
                if ( vx0 <= vx && vx < vx1 && vy0 <= vy && vy < vy1) {        //像素点在指定的范围内
                    c = buf[by * sht->bxsize + bx];
                    if(c != sht->col_inv){
                        vram[ctl->xsize*vy + vx] = c;    //不是特透明色的部分，都传递给vram
                    }
                }
            }
        }
